...
function CleverTabs(self, options) {
    ...
    this.options = $.extend({
        ...
        //Tab用于控制的头模板
        tabHeaderTemplate: '<li id="cleverTabHeaderItem-#{id}" class="#{liclass}"><a href="javascript:void(0)" title="#{title}">#{label}</a></li>',
        //Tab用于显示的Panel模板
    ...
    self.addClass("ui-tabs");
    this.wrapper = self;
    var el = self.find('ol,ul').eq(0);
    this.element = el;
    this.element.addClass("ui-tabs-nav ui-helper-reset ui-helper-clearfix ui-corner-all");
...
CleverTabs.prototype.add = function (options) {
	...
	var exsitsTab = self.getTabById(options.id);
    if (exsitsTab) {
        if (!exsitsTab.activate()) {
        	if(options.url!=exsitsTab.url){
        		$("#cleverTabHeaderItem-"+options.id).find("a:first").text(options.label.length > 7?(options.label.substring(0, 7) + '...'):options.label);
        		exsitsTab.setUrl(options.url);
        		exsitsTab.refresh();
        	}
        	return false;
        }
...
CleverTabs.prototype.setupContextMenu = function () {
    ...
    if (!self.options.contextMenu) {
        ...
        + '<li id="mnuEnabl" ><a href="#enabled"><span class="ui-icon ui-icon-pencil" style="float: left; margin-right: 5px;"></span>可用</a></li>'
        + '<li id="mnuDisalb"><a href="#disable"><span class="ui-icon ui-icon-cancel" style="float: left; margin-right: 5px;"></span>不可用</a></li>'
        ...
        + '</ul>'),
            handler: function (action, el, pos, srcElement) {
        		var tab,lid;
        		if(srcElement){
        			lid = srcElement.closest("li").attr("id");
        		}
        		if(lid){
        			lid = CleverTab.getUniqueByHeaderId(lid);
        			tab = self.getTabById(lid);
        		}else{
...
//获取指定Id的Tab
CleverTabs.prototype.getTabById = function (id) {
	if (!id) {
		return null;
	}
	var self = this;
	var frame = self.panelContainer.find("#cleverTabPanelItem-"+id+">iframe");
	var tab;
	if(frame.size()>0){
		tab = new CleverTab(self, CleverTab.getUniqueByPanelId(id));
	}
	return tab;
}
//获取指定Url的Tab
...
//为Tab页面设置新的url
CleverTab.prototype.setUrl = function (url) {
	if(url){
		this.url = url;
	}
}
